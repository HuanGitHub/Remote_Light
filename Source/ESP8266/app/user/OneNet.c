...
char* src_dev = "29636917";
char* dst_dev = "nskprOkYKdfqCmW6hJKSKwkTAY4=";

LOCAL os_timer_t T_OneNet_HeartTime;
void OneNet_Deal_CMDREQ();
char OneNet_GPIO_Control(const char *req);
...
//	os_printf("_write_pos:%d\r\n ",pkg->_write_pos);
	...
	use_Timer(&T_OneNet_HeartTime,OneNet_Tcp_Cline_HeartBeat,NULL,10000,1);
}
...
//	os_printf("Begin anayl data");
	...
	WriteBytes(recv_buf, data, len);
	 if ((pkg = GetEdpPacket(recv_buf)) == 0)
     {
	  ...
	  switch (mtype)
	 {
	 	case CONNRESP:		os_printf("recv connect resp, rtn: %d\n", UnpackConnectResp(pkg));
                			break;
	...
	if (UnpackCmdReq(pkg, &cmdid, &cmdid_len,
					&cmd_req, &cmd_req_len) == 0){
						 ...
						 */
						 cmd_req +=8;
					//	os_printf("cmd_req: %s\r\n",cmd_req);
						if(OneNet_GPIO_Control(cmd_req)){
						//	os_printf("\r\nOneNet req data:%x\r\n",*cmd_req);
